...
def driver_historical_completed_bookings(df: pd.DataFrame) -> pd.DataFrame:
    try:
        #historic completed information will not be present in the test file. In production environment I would want it to be update everyday for every driver. 
        #So that we can pick it directly. except block is to handle the test data scenario, as no historic information is available in test file.
